        ...
        <section class="panel">
                            ...
                            <tbody>
                                ...
                                foreach ($categories as $category) { ?>
                                    <tr>
                                        ...
                                        <td data-title="Full Name"><?php echo $category['Category']['name'] ; ?></td>
                                        <td data-title="Created"><?php echo date('F d,Y', strtotime($category['Category']['created'])); ?></td>
                                        <td data-title="Action">
                                            <?php if(ClassRegistry::init('Admin')->getCheckRolesSub($adminLId, $checkSubRols, 4, 2)){ ?>
                                            ...
                                            <?php echo $this->Html->link('<i class="fa fa-pencil"></i>', array("controller" => "categories", "action" => 'editcategory',$category['Category']['slug']), array('escape' => false, 'class' => "btn btn-warning btn-xs", 'title' => 'Edit')); 
                                            }
                                            if(ClassRegistry::init('Admin')->getCheckRolesSub($adminLId, $checkSubRols, 4, 3)){ 
                                             echo $this->Html->link('<i class="fa fa-trash-o "></i>', array('controller' => 'categories', 'action' => 'deleteCategory',$category['Category']['slug']), array('update' => 'deleted' . $category['Category']['id'], 'indicator' => 'loaderID', 'class' => 'btn btn-primary btn-xs', 'confirm' => 'Are you sure you want to Delete ?', 'escape' => false, 'title' => 'Delete'));
                                            }
                                            if(ClassRegistry::init('Admin')->getCheckRolesSub($adminLId, $checkSubRols, 4, 1)){ 
                                                echo $this->Html->link('<i class="fa fa-plus"></i>', array("controller" => "categories", "action" => 'subindex/', $category['Category']['slug']), array('escape' => false, 'class' => "btn btn-warning btn-xs", 'title' => 'Sub Category List'));
                                            }
                                            ?>
                                        </td>
                                    </tr>
            ...
            <div id="actdiv" class="outside">
                <div class="block-footer mogi">
                    ...
                    <?php
                    if(ClassRegistry::init('Admin')->getCheckRolesSub($adminLId, $checkSubRols, 4, 2)){ 
                        echo $this->Ajax->submit("Activate", array('div' => false, 'url' => array('controller' => 'categories', 'action' => 'index'), 'update' => 'listID', 'indicator' => 'loaderID', 'before' => "setAction('activate');", 'confirm' => "Are you sure you want to Activate ?", 'condition' => "isAnySelect(this.form)", "complete" => "showMessage('activated');", 'class' => 'btn btn-success btn-cons')); 
                        echo $this->Ajax->submit("Deactivate", array('div' => false, 'url' => array('controller' => 'categories', 'action' => 'index'), 'update' => 'listID', 'indicator' => 'loaderID', 'before' => "setAction('deactivate');", 'confirm' => "Are you sure you want to Deactivate ?", 'condition' => "isAnySelect(this.form)", "complete" => "showMessage('deactivated');", 'class' => 'btn btn-success btn-cons'));
                    }
                    if(ClassRegistry::init('Admin')->getCheckRolesSub($adminLId, $checkSubRols, 4, 3)){ 
                        echo $this->Ajax->submit("Delete", array('div' => false, 'url' => array('controller' => 'categories', 'action' => 'index'), 'update' => 'listID', 'indicator' => 'loaderID', 'before' => "setAction('delete');", 'confirm' => "Are you sure you want to Delete ?", 'condition' => "isAnySelect(this.form)", "complete" => "showMessage('deleted');", 'class' => 'btn btn-success btn-cons'));
                    }
                    ?>
                </div>
            </div>
            ...
            if (isset($searchKey) && $searchKey != '') {
                echo $this->Form->hidden('Category.name', array('type' => 'hidden', 'value' => $searchKey));
            }
...
<?php } else { ?>
    <div class="columns mrgih_tp">
        <table class="table table-striped table-advance table-hover table-bordered">
            <tr>
                <td><div id="noRcrdExist" class="norecext">There are no Category to show.</div></td>
            </tr>
        </table>
    </div>
    <?php
