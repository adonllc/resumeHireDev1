...
class PImageTestComponent extends Component {
    ...
    public function optimizeImage($sourcePath, $destinationPath, $imageName, $boxW, $boxH, $quality=80, $unlink=0){
        $imagePath = $sourcePath.$imageName;
        $imagePathDest = $destinationPath.$imageName;
        list($width, $height, $type, $attr) = getimagesize($imagePath);
        if($width >= $height){
            if($sourcePath != $destinationPath){
                @unlink($imagePathDest);
                copy($imagePath,$imagePathDest);
            }
            if($width > $boxW){
               $this->resize($imagePath, $imagePathDest, $boxW, '', 80);
            }
        }else{
            if($sourcePath != $destinationPath){
                @unlink($imagePathDest);
            ...
            if($height > $boxH){
               $this->resize($imagePath, $imagePathDest, '', $boxH , 80);
            }
        }
        if($unlink){
            @unlink($imagePath);
