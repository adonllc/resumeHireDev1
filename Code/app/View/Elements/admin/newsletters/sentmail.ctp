...
<?php if ($sendmails) { ?>
    <div class="col-lg-12">
        <section class="panel">
            ...
            <?php
            $urlArray = array_merge(array('controller' => 'newsletters', 'action' => 'sentMail', $separator));
            $this->Paginator->_ajaxHelperClass = "Ajax";
            $this->Paginator->Ajax = $this->Ajax;
            $this->Paginator->options(array('update' => 'listID', 'url' => $urlArray, 'indicator' => 'loaderID'));
            ?>
            <?php echo $this->Form->create("Sendmail", array("url" => "sentMail", "method" => "Post")); ?>
            <div class="columns mrgih_tp">
                <div id="pagingLinks" align="right">
                    ...
                    <span class="custom_link pagination"> 
                        ...
                        <?php if ($this->Paginator->hasPrev('Sendmail')) echo $this->Paginator->prev('Prev', array()); ?>&nbsp;
                        <?php echo $this->Paginator->numbers(array('separator' => '  ')); ?>&nbsp;
                        <?php if ($this->Paginator->hasNext('Sendmail')) echo $this->Paginator->next('Next', array()); ?>&nbsp;
                        ...
                        <table class="table table-bordered table-striped table-condensed cf">
                            <thead class="cf">
                                <tr>
                                    ...
                                    <th class="sorting_paging"><?php echo $this->Paginator->sort('Sendmail.email', 'Email Address'); ?></th>
                            ...
                            <tbody>
                                <?php foreach ($sendmails as $newsletter) { ?>
                                    <tr>
                                        <td data-title=""><input type="checkbox" onclick="javascript:isAllSelect(this.form);" name="chkRecordId" value="<?php echo $newsletter['Sendmail']['id']; ?>" /></td>
                                        <td data-title="Email Address"><?php echo $newsletter['Sendmail']['email']; ?></td>
                                        <td data-title="Email Address">
                                            <?php
                                            $userType = classregistry::init('User')->field('user_type', array('User.email_address' => $newsletter['Sendmail']['email']));
                                        ...
                                        <td data-title="Mail Sent Time"><?php
                                            if ($newsletter['Sendmail']['sent_on'] != '' && $newsletter['Sendmail']['sent_on'] != '0' && $newsletter['Sendmail']['sent_on'] != NULL) {
                                                echo date('F d,Y H:i:s', $newsletter['Sendmail']['sent_on']);
                                            } else {
                                                echo 'N/A';
                                            }
                                            ?></td>
                                        <td data-title="Action">
                                            <div id="loaderIDAct<?php echo $newsletter['Sendmail']['id']; ?>" style="display:none;position:absolute;margin:0px 0 0 4px;z-index: 9999;"><?php echo $this->Html->image("loading.gif"); ?></div>
                                            <span id="status<?php echo $newsletter['Sendmail']['id']; ?>">
                                                <?php
            ...
            <div id="actdiv" class="outside">
                <div class="block-footer mogi">
                    <?php echo $this->Form->text('Sendmail.idList', array('type' => 'hidden', 'value' => '', 'id' => 'idList')); ?>
                    <?php echo $this->Form->text('Sendmail.action', array('type' => 'hidden', 'value' => 'activate', 'id' => 'action')); ?>
            ...
            if (isset($searchKey) && $searchKey != '') {
                echo $this->Form->hidden('Sendmail.email', array('type' => 'hidden', 'value' => $searchKey));
            }
            ?>
