<?php
	...
	class UploadComponent extends Component {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('application/x-xls','application/msexcel ','application/pdf','application/xls ','text/plain','application/octet-stream','application/word ','application/vnd.ms-excel','application/winword ','appl/text','application/doc','application/msword','zz-application/zz-winassoc-psd','application/psd','application/photoshop','image/x-vnd.adobe.photoshop','image/psd','image/x-photoshop','image/photoshop','image/jpg','image/jpeg','image/pjpeg','image/gif','image/png','image/bmp','application/zip'); }
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				if(!in_array($fileType,$this->_allowed)){
					...
					$this->_result['errormsg']		= "Invalid File Type (Allowed File types are: jpg,jpeg,gif,png)";
					return $this->_result;
